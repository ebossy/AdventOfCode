...
def get_score(i, j, height,already_visited_max = []):
    if height == 9 and (i,j) not in already_visited_max:
        already_visited_max.append((i,j))
        return 1
    summary = 0
    ...
    for move in directions:
        ...
        if 0 <= ix < len(hike_map) and 0 <= jx< len(hike_map[0]):
            if hike_map[ix][jx] == height+1:
                summary+=get_score(ix, jx, height+1, already_visited_max)
